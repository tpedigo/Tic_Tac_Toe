...
class Gameboard < GameSquare
    ...
    def player_choice
        ...
        while choosing
            ...
            position = gets.chomp
            if ["0", "1", "2", "3", "4", "5", "6", "7", "8"].include?(position) && @gameboard[position.to_i].play == position
                choosing = false
            else
                puts "Not a valid input. Please select an open square 0 through 8."
                choosing = true
            end
        end
        choosing = true
    ...
    def update_board
        ...
        for i in 0..8 do
            if i.to_s == position && ["0", "1", "2", "3", "4", "5", "6", "7", "8"].include?(@gameboard[i].play)
    ...
    def check_board
        if (@gameboard[0].play != "0") && ((@gameboard[0].play == @gameboard[1].play && @gameboard[1].play == @gameboard[2].play) || (@gameboard[0].play == @gameboard[3].play && @gameboard[3].play == @gameboard[6].play) || (@gameboard[0].play == @gameboard[4].play && @gameboard[4].play == @gameboard[8].play))
            ...
            return false
        elsif @gameboard.all? { |square| square.play == "X" || square.play == "O" }
            puts "Tie game! No moves left."
            return false
        else
            puts "Next round!"
            return true
